...
function App() {
  ...
  useEffect(() => {
    // 1. Al cargar la app por primera vez, asignamos la pestaña inicial UNA SOLA VEZ
    supabase.auth.getSession().then(({ data: { session } }) => {
      setSession(session);
      if (session?.user?.email) {
        const esAdmin = session.user.email.includes('admin');
        setRol(esAdmin ? 'admin' : 'visita');
        setActiveTab(esAdmin ? 'registro' : 'colmena'); 
      }
    });

    // 2. Escuchamos cambios de seguridad (Supabase refrescando conexión)
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      if (session?.user?.email) {
        const esAdmin = session.user.email.includes('admin');
        setRol(esAdmin ? 'admin' : 'visita');
        // ¡Magia! Aquí ya NO modificamos 'activeTab', así que no te jalará de regreso.
      } else {
        setRol(null);
      }
    });
    ...
    <div className="h-screen w-full bg-slate-50 flex flex-col overflow-hidden">
      ...
      <header className="bg-white border-b p-4 px-8 flex justify-between items-center shadow-sm shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-extrabold text-xl text-slate-800">BioMonitor PRUEBA</h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              {rol === 'admin' ? 'Administrador' : 'Visitante'}
            </p>
          </div>
        ...
        <nav className="flex bg-slate-200/60 rounded-2xl p-1 gap-1">
          ...
          <button onClick={() => cambiarPestana('colmena')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'colmena' ? 'bg-amber-500 text-white shadow-md' : 'text-slate-500 hover:bg-white'}`}><Bug size={16} /> Colmena</button>
          <button onClick={() => cambiarPestana('acuaponia')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'acuaponia' ? 'bg-cyan-500 text-white shadow-md' : 'text-slate-500 hover:bg-white'}`}><Fish size={16} /> Acuaponía</button>
          <button onClick={() => cambiarPestana('hidroponia')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'hidroponia' ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-500 hover:bg-white'}`}><Leaf size={16} /> Hidroponía</button>
        </nav>
